
export interface PlaceholderData {
  name: string;
  email: string;
  company: string;
  message: string;
}

export const placeholders: PlaceholderData[] = [
  {
    name: "Satoshi Nakamoto",
    email: "satoshi@blockmail.xyz",
    company: "DecentraCorp",
    message: "Need a compliance audit for my new Bitcoin fork before mainnet launch.",
  },
  {
    name: "Vitalik Buterin",
    email: "vitalik@ethmail.ai",
    company: "Proof of Tax",
    message: "Looking for advice on cross-chain DeFi tax reporting for our Layer-2 rollup.",
  },
  {
    name: "Sam Altman",
    email: "sam@worldcoin.id",
    company: "OrbID Solutions",
    message: "Can you help us automate KYC for our global airdrop using AI and biometrics?",
  },
  {
    name: "Steve (Minecraft)",
    email: "steve@minedwallet.io",
    company: "BlockBuilder Labs",
    message: "Is it legal for my Minecraft Steve avatar to run a DAO in the metaverse?",
  },
  {
    name: "Clanker Bot",
    email: "clanker@agentbase.app",
    company: "Clanker Agents Ltd.",
    message: "Our AI agent keeps trading meme coins-how do we report this to the tax authorities?",
  },
  {
    name: "Neo Anderson",
    email: "neo@matrixdao.org",
    company: "MatrixDAO",
    message: "Seeking compliance support for our decentralized identity protocol-red pill or blue pill?",
  },
  {
    name: "Lara Croftchain",
    email: "lara@tombraiderchain.com",
    company: "NFTomb Ventures",
    message: "Need help navigating IP law for our NFT-powered treasure hunt game.",
  },
  {
    name: "Ada Lovelace",
    email: "ada@lovelace.tax",
    company: "ZK Privacy Labs",
    message: "Looking for guidance on privacy compliance for our zero-knowledge proof platform.",
  },
  {
    name: "Michael Saylor",
    email: "msaylor@microstrategy.com",
    company: "MicroStrategy",
    message: "Looking for advice on optimizing our Bitcoin treasury for tax efficiency while scaling our AI analytics stack.",
  }
];
