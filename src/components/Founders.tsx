  ...
  return <section id="founders" className="py-20 bg-gradient-to-b from-[#fbf9ff] to-[#fdf5fa] relative">
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {founders.map((founder, index) => <div 
            key={index} 
            className={`team-card p-6 backdrop-blur-sm hover:translate-y-[-4px] transition-all duration-300 text-center rounded-lg ${isDarkBackground ? 'bg-white/10' : 'bg-white/95'}`}
            style={{
          ...
          >
              ...
              <h3 className={`text-xl font-semibold ${isDarkBackground ? 'text-white' : 'text-gray-800'}`}>{founder.name}</h3>
              <div className="mt-1 text-sm font-medium text-[#cb46b3]">{founder.role}</div>
              
              <p className={`mt-3 text-sm relative ${isDarkBackground ? 'text-gray-200' : 'text-gray-600'}`}>
                {founder.bio}
                <Dialog>
